...
/// Locales added as optional argument
String formatDate(DateTime date, List<String> formats, {Map locale = locale}) {
	...
	for (String format in formats) {
		if (format == yyyy) {
			...
			sb.write(locale['monthLong'][date.month - 1]);
		} else if (format == M) {
			sb.write(locale['monthShort'][date.month - 1]);
		} else if (format == dd) {
			...
			sb.write(locale['dayLong'][date.weekday - 1]);
		} else if (format == D) {
			sb.write(locale['dayShort'][date.weekday - 1]);
		} else if (format == HH) {
